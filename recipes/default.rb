...
when 'rhel', 'fedora'
  ...
  ruby_block "reboot into openvz kernel" do
    block do
      shell_out!("sync; sync; sync; shutdown -r now")
      Chef::Application.fatal!("going down for reboot")
    end
    action :nothing
  end
  ...
  execute "vdc-manage network dhcp addrange nw-demo1" do
    command <<-CMD
      /opt/axsh/wakame-vdc/dcmgr/bin/vdc-manage network dhcp addrange nw-demo1 192.168.3.1 192.168.3.254
    CMD
    action :nothing
    notifies :run, 'execute[vdc-manage network reserve nw-demo1]', :immediately
  end

  execute "vdc-manage network reserve nw-demo1" do
    command <<-CMD
  ...
  execute 'vdc-manage network dc add public --uuid dcn-public' do
    command <<-CMD.gsub(/^ {6}/,'')
      /opt/axsh/wakame-vdc/dcmgr/bin/vdc-manage -e <<CMDSET
